...
class Inventory:
    def __init__(self):
        self.items=[["None",0],["None",0],["None",0],["None",0],["None",0],["None",0],["None",0],["None",0]]#Item,Count
        self.selected=0
        ...
        for i in range(size):
            ...
            if transform==0:
                ...
                if global_array[px][py]!=0:
                    global_array[px][py].item=transform
                    global_array[px][py].reset_health()


def generate_caves(total,minH,maxH,length,width):
    #Give weight to certain direction
    #Length of cave and width, will cross into others to create caves
    global global_array
    
    for i in range(total):
        px,py=random.randint(0,999),random.randint(minH,maxH)
        move_arrayx=[-1,0,1]
        move_arrayy=[-1,0,1]
    ...
    for y in range(blocksy):
        if y!=0 and y!=blocksy:
            for x in range(blocksx):
                if global_array[x][y]!=0:
                    if x!=9 and x!=blocksx-1:
                        if global_array[x-1][y]==0 and global_array[x+1][y]==0 and global_array[x][y-1]==0 and global_array[x][y+1]==0:
                            global_array[x][y]=0
...
def create_layer(min,max,item):#Use to create a layer of blocks
    for y in range(max-min):
        for x in range(blocksx):
            global_array[x][y+min].item=item
        ...
        for x in range(blocksx):
            ...
            if rando<odds:
                if global_array[x][max+y]!=0:
                    global_array[x][max+y].item=item
    ...
    if global_array[x][y].item=="None":
        ...
        if not (mousex>152 and mousex<168 and mousey>82 and mousey<98):
            if inventory.items[inventory.selected]!="None":
                global_array[x][y].item=inventory.items[inventory.selected][0]
...
#Stone layer
create_layer(0,950,"Stone")

generate_caves(1000,20,1000,300,10)#LONG CAVES
generate_veins("None",1000,0,1000,0,900,200)#HOLLOW CAVES

generate_veins("Dirt",3000,0,1000,0,900,200)
generate_veins("Stone",500,950,1050,960,1000,100)
generate_veins("Iron",1000,0,1000,0,950,20)
generate_veins("Lead",1000,0,900,0,950,20)
generate_veins("Copper",1000,0,900,0,950,10)
...
#Get font to display small text in game

#Add grass to world and trees
#Add parallax background and walls to caves
#Remove blocks that are alone
